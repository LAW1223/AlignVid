...
def main(args):
    ...
    @torch.no_grad()
    def worker(input_image, sample_id, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs,
               rs, gpu_memory_preservation, use_teacache, control_scale, warmup=False):
        """
        ...
        for latent_padding in latent_paddings:
            ...
            if history_pixels is None:
                ...
                curr_pixels = vae_decode(real_lat[:, :, :latent_window_size * 2 + (1 if is_last_section else 0)],
                                         vae).cpu()
    ...
    for sample in samples:
        start_time = time.time()
        pil_img = Image.open(os.path.join(args.data_dir, sample['image_name'])).convert("RGB")
        if args.blur_img:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--length', type=float, default=5.0, help='Total video length in seconds')
...
torchrun --nproc-per-node=1 --nnodes=1 --master_port=39504 vbench_v1_json.py \
  --control_scale 1.0 \
  --data_dir /path/to/project/OmitI2V/evaluation/tool_based/vbench2_beta_i2v/data/crop/16-9 \
  --prompt_file /path/to/project/OmitI2V/evaluation/tool_based/vbench2_beta_i2v/Vbench_full_info/VBench_full_info_part1.json \
  --output_dir /path/to/project/OmitI2V_Results/videos/FramePack_v1/vbench_original
'''