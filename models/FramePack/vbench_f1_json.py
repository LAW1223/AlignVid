...
def main(args):
    ...
    transformer = HunyuanVideoTransformer3DModelPacked.from_pretrained(
        "lllyasviel/FramePack_F1_I2V_HY_20250503", torch_dtype=torch.bfloat16).cpu()
    ...
    def worker(input_image, sample_id, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs,
        ...
        print(
            f"Total latent sections: {total_latent_sections}, Total frames: {total_latent_sections * latent_window_size * 4 - 3}")
...
torchrun --nproc-per-node=1 --nnodes=1 --master_port=39551 vbench_f1_json.py \
  --control_scale 1.35 \
