...
> **Note (AlignVid fork).** The interactive Gradio GUI (`demo_gradio.py`) and the
> standalone CLI/serving demos from upstream are not included in this release —
> this fork ships only the JSON-driven AlignVid inference scripts
> (`inference_f1_json.py`, `inference_v1_json.py`). For the original GUI, see the
> upstream [FramePack](https://github.com/lllyasviel/FramePack) repository.
