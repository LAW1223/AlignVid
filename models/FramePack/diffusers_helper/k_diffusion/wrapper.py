...
def fm_wrapper(transformer, t_scale=1000.0):
    def k_model(x, sigma, **extra_args):
        ...
        control_scale = extra_args['control_scale']
        ...
        pred_positive = transformer(hidden_states=hidden_states, timestep=timestep, return_dict=False, control_scale=control_scale, **extra_args['positive'])[0].float()

        if cfg_scale == 1.0:
            pred_negative = torch.zeros_like(pred_positive)
        else:
            pred_negative = transformer(hidden_states=hidden_states, timestep=timestep, return_dict=False, control_scale=control_scale, **extra_args['negative'])[0].float()
